...
/*
  Try to find to what identity the message was sent.
  First compare mail.mbox_id to available identities,
  then if no success compare the To and Cc headers to identities.
  Returns the identity number, 0 if no identity found, else the identity number
*/
QString
mail_msg::lookup_dest_identity()
{
  int m_ident=0;
  QString result;
  static mailboxes mboxes;	// NON MT-SAFE
  static identities m_ids;	// NON MT-SAFE
  if (!mboxes.fetch() || !m_ids.fetch())
    return result;
  int mb_id=mbox_id();
  if (mb_id) {
    DBG_PRINTF(3, "mbox_id=%d for mail_id=%u\n", mb_id, get_id());
    const QString& mbox_name=mboxes.name_by_id(mb_id);
    if (!mbox_name.isEmpty()) {
      //      DBG_PRINTF(3, "mbox_name=%s\n", (const char*)mbox_name.local8Bit());
      identities::const_iterator ii = m_ids.find(mbox_name);
      if (ii!=m_ids.end()) {
	m_ident=ii->second.m_internal_id;
	DBG_PRINTF(3, "m_ident=%d\n", m_ident);
	result = mbox_name;
      }
      else {
	DBG_PRINTF(3, "identity not found\n");
      }
    }
  }

  if (m_ident==0) {
    // No mb_id: this is probably because the mail has been imported with an older
    // version of manitou-mdx or from a mailbox file
    ...
    for (iter1 = emails_list.begin(); iter1!=emails_list.end(); ++iter1) {
      ...
      if (iit != m_ids.end()) {
	m_ident=iit->second.m_internal_id;
	result = *iter1;
	break;
      }
    }
...
mail_msg::setup_reply(const QString& quoted_text, int whom_to, body_format format)
{
  ...
  if (get_config().get_bool("self_cc_on_reply")) {
    if (!us.isEmpty()) {
      if (!replyHeader.m_cc.isEmpty())
	replyHeader.m_cc.append(", ");
      replyHeader.m_cc.append(us);
    }
  }
