...
class mime_msg_viewer: public QWidget
{
  Q_OBJECT
public:
  mime_msg_viewer(const char* rawmsg, const struct display_prefs& prefs);
...
private:
  /* Appends to 'html_output' an html representation of 'body' which is
     ...
     come along, FIXME) */
  void format_body(QString& html_output, const char* body, const struct display_prefs& prefs);
