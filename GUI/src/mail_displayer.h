...
public:
  ...
  QString expand_body_line(const QString&, const struct display_prefs& prefs);
  bool m_wrap_lines;
  ...
  QString text_body_to_html(const QString &b, const struct display_prefs& prefs);
  QString format_filters_trace(const filter_log_list&);
  QString sprint_additional_headers(const struct display_prefs& prefs,
				    mail_msg* msg);
