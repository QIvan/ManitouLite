...
msg_list_window::msg_list_window (const msgs_filter* filter, struct display_prefs* dprefs, QWidget *parent)  : QMainWindow(parent)
{
  m_abort = false;
...
//  topLayout->addWidget(m_tags_box);
  if (dprefs) {
    ...
    display_vars.m_threaded = get_config().get_bool("display_threads");
    display_vars.m_wrap_lines = true;
    display_vars.m_show_tags = get_config().get_bool("show_tags");
    display_vars.m_hide_quoted = 0;
    display_vars.m_clickable_urls=get_config().get_bool("body_clickable_urls", true);
    display_vars.m_show_filters_trace = get_config().get_bool("display/filters_trace");
    display_vars.m_show_tags_in_headers = get_config().get_bool("display/tags_in_headers", true);
  }
...
{
  if (title.isEmpty()) {
    QString sTitle=QString("%3: %1 %2").arg(m_filter->m_list_msgs.size()).arg(tr("message(s)")).arg(db_cnx::dbname());
...
#if 0  // pre-Qt4
  // uncheck all
  m_pPopupHeaders->setItemChecked(m_menuItemId[me_Display_Headers_None],FALSE);
  m_pPopupHeaders->setItemChecked(m_menuItemId[me_Display_Headers_Most],FALSE);
  m_pPopupHeaders->setItemChecked(m_menuItemId[me_Display_Headers_All],FALSE);
  m_pPopupHeaders->setItemChecked(m_menuItemId[me_Display_Headers_Raw],FALSE);
  m_pPopupHeaders->setItemChecked(m_menuItemId[me_Display_Headers_RawDec],FALSE);
  // check the one
  m_pPopupHeaders->setItemChecked(which,TRUE);
#endif
...
msg_list_window::mail_reply(int whom_to)
{
  ...
  if (m_msgview->content_type_shown() <= 1) {
	...
	text_to_quote = m_msgview->selected_html_fragment(true);
      }
      else {
...
msg_list_window::check_new_mail()
{
  try {
    ...
    db_cnx db;
    sql_stream query_stream(query, db);

    m_auto_refresh_results.clear();
    m_filter->load_result_list(query_stream, &m_auto_refresh_results);
...
msg_list_window::timer_func()
{
  ...
  if (m_waiting_for_results && m_thread.isFinished()) {
    ...
    if (m_loading_filter->m_fetch_results) {
      // display the results
#if 0
      if (nb_messages()==0) {
	// reuse our window if it's empty (whatever the preferred open mode)
	fill_fetch(m_loading_filter);
      }
      else
#endif
      {
...
#if 0
    // Check for new mail at some intervals
    if (m_filter->auto_refresh()) {
      int delay=get_config().get_number("autocheck_newmail"); // minutes
      if (delay!=0 && (m_timer_ticks/5)%(delay*60)==0)
	check_new_mail();
    }
#endif
