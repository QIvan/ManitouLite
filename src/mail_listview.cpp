...
mail_item_model::insert_msg(mail_msg* msg, QStandardItem* parent/*=NULL*/)
{
  ...
  date_item* idate = new date_item(msg->msg_date().OutputHM(m_date_format));
  if (bold) {
    QFont f=idate->font();
    f.setBold(true);
    idate->setFont(f);
  ...
  QVariant vdate;
  vdate.setValue(msg->msg_date());
  idate->setData(vdate, mail_item_model::mail_date_role);
...
mail_listview::mail_listview(QWidget* parent): QTreeView(parent)
{
  ...
  if (d=="DD/MM/YYYY HH:MI")
    ...
    m_date_format=1;
  model->set_date_format(m_date_format);
...
mail_listview::make_header_menu(QWidget* parent)
{
  ...
  for (int i=0; i<mail_item_model::ncols; i++) {
    QAction* a = menu->addAction(tr(m_column_names[i]));
//    DBG_PRINTF(1, "action added=%p, [%s]", a, m_column_names[i]);
    a->setCheckable(true);
...
mail_listview::popup_ctxt_menu(const QPoint& pos)
{
  ...
  if (action==NULL) {
    ...
    m_msg_window->current_filter()->fetch(this, true);
...
mail_listview::init_columns()
{
  ...
  static const int default_sizes[] = {280,170,32,32,24,24,115};
  QString s;
