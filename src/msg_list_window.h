...
{
  Q_OBJECT
  struct display_prefs display_vars;

public:
  msg_list_window (const msgs_filter*, struct display_prefs* dprefs=0, QWidget *parent = 0);
  virtual ~msg_list_window ();

  void propagate_status(mail_msg*, int code=0);

  const struct display_prefs& get_display_prefs();
