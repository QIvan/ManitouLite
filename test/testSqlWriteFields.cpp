...
void test_sql_write_fields::insert()
{
    try{
        ...
        sql_stream check("SELECT * from test_table",*m_DB);
        int id, int_not_null;
        QString text, char200, date;
        check >> id >> text >> char200 >> date >> int_not_null;
        CPPUNIT_ASSERT(id == 1);
        ...
        QString date2;
        check >> id >> text >> char200 >> date2 >> int_not_null;
        CPPUNIT_ASSERT(id == 2);
        CPPUNIT_ASSERT(text == "Insert Te");
        CPPUNIT_ASSERT(char200 == "Insert Text 2");
        CPPUNIT_ASSERT(date != date2);
